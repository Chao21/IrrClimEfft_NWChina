#--coding:utf-8--
# Zhang Chao 2022/4/8
# correlation analysis between albedo and LST in daytime and nighttime
...
def getMeanSe(df,var):
    Mean = list(df.groupby(['Albedointerval'])[var].mean())
    count = list(df.groupby(['Albedointerval'])[var].count())
    std = list(df.groupby(['Albedointerval'])[var].std())
...
dff['Albedointerval'] = pd.cut(dff['Albedo'],[-1,0,0.01,10],
                           labels=[0,0.01,10])
dfg['Albedointerval'] = pd.cut(dfg['Albedo'],[-1,0,0.01,10],
                           labels=[0,0.01,10])
dfu['Albedointerval'] = pd.cut(dfu['Albedo'],[-1,0,0.01,10],
                           labels=[0,0.01,10])
...
def DrawsubPlots(ax,group,df,varX,varY,No,title,Ylim,xtickabel,
                 ytick,yticklabel,xlabel,ylabel,insetYtick,insetYlim):
    x = [1, 3, 5]
    y = group[0]
    ye = group[1]
    ...
    rgb = '#5C2366' ##C15555  5C2366
    plt.sca(ax)
    plt.bar(x, y, alpha=.9, width=1, color=rgb, label="forest2crop", yerr=ye, error_kw=err_attri,zorder=10,)#C15555  5C2366
    plt.ylim(Ylim)
    ...
    axins.scatter(X, Y, s=10, c=rgb, marker='.', alpha=0.8)
    ...
    axins.set_xlabel('$\Delta$Albedo',labelpad = 0.5 ,fontsize = 10)#
...
Ylim1 = [-3.7,0.3]
Ylim2 = [-1.8,0.3]
Xticklabel = ['<0','0~0.01','>0.01']
Ytick1 = [-3,-2,-1,0]
Yticklabel1 = [-3,-2,-1,0]
Ytick2 = [-1.0,-0.5,0]
Yticklabel2 = [-1.0,-0.5,0]

Xlabel = '$\Delta$Albedo'
Ylabel1 = 'Daytime $\Delta$LST (K)'#$^\circ$C
Ylabel2 = 'Nighttime $\Delta$LST (K)'#$^\circ$C

DrawsubPlots(ax1,LSTDayf,dff,'Albedo','LSTDay','(a)','forest2crop',Ylim1,'',Ytick1,Yticklabel1,'',Ylabel1,[-3,0,3],[-8,4])
DrawsubPlots(ax4,LSTNightf,dff,'Albedo','LSTNight','(d)','',Ylim2,Xticklabel,Ytick2,Yticklabel2,'',Ylabel2,[-2,0,2],[-3,2.5])

DrawsubPlots(ax2,LSTDayg,dfg,'Albedo','LSTDay','(b)','grass2crop',Ylim1,'',Ytick1,'','','',[-6,0,],[-12,4])
DrawsubPlots(ax5,LSTNightg,dfg,'Albedo','LSTNight','(e)','',Ylim2,Xticklabel,Ytick2,'',Xlabel,'',[-2,0,2],[-3,2.5])

DrawsubPlots(ax3,LSTDayu,dfu,'Albedo','LSTDay','(c)','unuse2crop',Ylim1,'',Ytick1,'','','',[-6,0],[-12,4])
DrawsubPlots(ax6,LSTNightu,dfu,'Albedo','LSTNight','(f)','',Ylim2,Xticklabel,Ytick2,'','','',[-2,0,2],[-3,2.5])
#
