#--coding:utf-8--
# Zhang Chao 2022/4/8
# Correlation analysis of LST and Albedo in daytime and nighttime in three regions
...
outname1 = outdir + r'\FigS11.svg'
outname2 = outdir + r'\FigS12.svg'
...
def DrawsubPlots(ax,X,Y,No,title,xticks,ylim,color,xlabel,ylabel,fenqu):
    plt.sca(ax)
...
# #########################################################################
# #########################################################################
#plot the scatterplot of LSTDay vs. Albedo
...
DrawsubPlots(ax1,Albedo_f2c[0].iloc[:,0].to_list(),LSTDay_f2c[0].iloc[:,0].to_list(),'(a)',
             'forest2crop',[0,0.02,0.04],[-4,3],colors[0],'','','')
DrawsubPlots(ax4,Albedo_f2c[1].iloc[:,0].to_list(),LSTDay_f2c[1].iloc[:,0].to_list(),'(d)',
             '',[-0.03,0.00,0.03],[-6,3],colors[1],'','$\Delta$LST (K)','')
DrawsubPlots(ax7,Albedo_f2c[2].iloc[:,0].to_list(),LSTDay_f2c[2].iloc[:,0].to_list(),'(g)',
             '',[0,0.02],[-3,1.5],colors[2],'','','')
DrawsubPlots(ax2,Albedo_g2c[0].iloc[:,0].to_list(),LSTDay_g2c[0].iloc[:,0].to_list(),'(b)',
             'grass2crop',[-0.03,0.00,0.03],[-8,4],colors[0],'','','')
DrawsubPlots(ax5,Albedo_g2c[1].iloc[:,0].to_list(),LSTDay_g2c[1].iloc[:,0].to_list(),'(e)',
             '',[-0.03,0.00,0.03],[-9,4],colors[1],'','','')
DrawsubPlots(ax8,Albedo_g2c[2].iloc[:,0].to_list(),LSTDay_g2c[2].iloc[:,0].to_list(),'(h)',
             '',[-0.03,0,0.03],[-3,1.0],colors[2],'$\Delta$Albedo','','')
DrawsubPlots(ax3,Albedo_u2c[0].iloc[:,0].to_list(),LSTDay_u2c[0].iloc[:,0].to_list(),'(c)',
             'unuse2crop',[-0.05,0.00],[-6,1.8],colors[0],'','','North Xinjiang')
DrawsubPlots(ax6,Albedo_u2c[1].iloc[:,0].to_list(),LSTDay_u2c[1].iloc[:,0].to_list(),'(f)',
             '',[-0.05,0.00],[-8,3],colors[1],'','','South Xinjiang')
DrawsubPlots(ax9,Albedo_u2c[2].iloc[:,0].to_list(),LSTDay_u2c[2].iloc[:,0].to_list(),'(i)',
             '',[-0.05,0],[-5,3],colors[2],'','','Hexi Corridor')
...
DrawsubPlots(ax1,Albedo_f2c[0].iloc[:,0].to_list(),LSTNight_f2c[0].iloc[:,0].to_list(),'(a)',
             'forest2crop',[0,0.02,0.04],[-1.8,0.8],colors[0],'','','')
DrawsubPlots(ax4,Albedo_f2c[1].iloc[:,0].to_list(),LSTNight_f2c[1].iloc[:,0].to_list(),'(d)',
             '',[-0.03,0,0.03],[-1.8,1.5],colors[1],'','$\Delta$LST (K)','')
DrawsubPlots(ax7,Albedo_f2c[2].iloc[:,0].to_list(),LSTNight_f2c[2].iloc[:,0].to_list(),'(g)',
             '',[0,0.03],[-0.7,0.4],colors[2],'','','')
DrawsubPlots(ax2,Albedo_g2c[0].iloc[:,0].to_list(),LSTNight_g2c[0].iloc[:,0].to_list(),'(b)',
             'grass2crop',[-0.03,0.00,0.03],[-2.0,1.2],colors[0],'','','')
DrawsubPlots(ax5,Albedo_g2c[1].iloc[:,0].to_list(),LSTNight_g2c[1].iloc[:,0].to_list(),'(e)',
             '',[-0.03,0.00,0.03],[-1.8,1.8],colors[1],'','','')
DrawsubPlots(ax8,Albedo_g2c[2].iloc[:,0].to_list(),LSTNight_g2c[2].iloc[:,0].to_list(),'(h)',
             '',[-0.03,0.00,0.03],[-1.3,1.2],colors[2],'$\Delta$Albedo','','')
DrawsubPlots(ax3,Albedo_u2c[0].iloc[:,0].to_list(),LSTNight_u2c[0].iloc[:,0].to_list(),'(c)',
             'unuse2crop',[-0.05,0.00],[-1.4,1.2],colors[0],'','','North Xinjiang')
DrawsubPlots(ax6,Albedo_u2c[1].iloc[:,0].to_list(),LSTNight_u2c[1].iloc[:,0].to_list(),'(f)',
             '',[-0.05,0.00],[-2.0,2.2],colors[1],'','','South Xinjiang')
DrawsubPlots(ax9,Albedo_u2c[2].iloc[:,0].to_list(),LSTNight_u2c[2].iloc[:,0].to_list(),'(i)',
             '',[-0.05,0.00],[-2,2],colors[2],'','','Hexi Corridor')
